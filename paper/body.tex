% =====================================================================
% body.tex -- shared content for the single-column (main.tex) and
% two-column (main_twocol.tex) builds. Everything except the preamble
% and \maketitle lives here. Wide floats use the widefigure/widetable
% environments, which each wrapper maps to figure/table or figure*/table*.
% =====================================================================
...
around force self-consistency, generalization across six distribution shifts,
...
head does not: all three sit at $\rho_s\approx0.95$--$0.999$, and a 29-parameter
...
settle, the surrogates were off by roughly $700\times$ their in-distribution
...
\begin{widefigure}
  \includegraphics[width=0.92\linewidth]{figures/fig01_schematic.pdf}
...
$4.8\times10^{-6}$ (median) on $\Cl$: roughly two orders of
...
1{,}281{,}701 (\Mtrans): a $\pm8\%$ spread, tighter than the budget requires.
...
symmetry residual ($4.5$, Table~\ref{tab:indist}), consistent with the
...
\begin{widefigure}
  \includegraphics[width=\linewidth]{figures/fig02_cp_profiles.pdf}
...
\begin{widefigure}
  \includegraphics[width=\linewidth]{figures/fig03_surface_fields.pdf}
...
On head-route $\Cd$ rank correlation the between-model gap stays under
$0.01$ on every split while the between-split gap for a single model reaches
$0.05$ ($0.999\to0.945$); architecture matters an order of magnitude less
than shift for the ranking metric. The ridge baseline is, again, the
...
\begin{widefigure}
  \includegraphics[width=\linewidth]{figures/fig04_error_vs_shift.pdf}
  \caption{Error versus distribution shift, one panel per OOD axis (scarcity,
           ...
           model, splits ordered along each axis; seed-0 runs. Field relative
           $L_2$ on surface pressure.}
...
fast as the field error it accompanies ($4.8\times$ vs.\ $3.6\times$ for
\Mgnn, $7.3\times$ vs.\ $5.9\times$ for \Mfno, $7.4\times$ vs.\ $7.5\times$
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/fig05_fsc_scatter.pdf}
...
\begin{widefigure}
  \includegraphics[width=0.75\linewidth]{figures/fig10_symmetry.pdf}
...
\begin{widetable}
  \input{tables/tab3_calibration}
\end{widetable}

\begin{widefigure}
  \includegraphics[width=\linewidth]{figures/fig06_reliability.pdf}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/fig07_coverage_vs_shift.pdf}
...
\begin{widefigure}
  \includegraphics[width=0.9\linewidth]{figures/fig08_interval_width.pdf}
...
point reused from the core grid: Figure~\ref{fig:data_efficiency}. Two things
...
The cost axis (Figure~\ref{fig:pareto}) makes the deployment reading direct:
at the full budget \Mgnn{} is Pareto-dominated, costing $2.9$--$3.2$ GPU-h to
...
\begin{widefigure}
  \begin{subfigure}{0.48\linewidth}\centering
    ...
    \caption{Data efficiency (log-log, seed bands).}\label{fig:data_efficiency}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\linewidth}\centering
    \includegraphics[width=\linewidth]{figures/fig12_cost_accuracy.pdf}
    \caption{Cost versus accuracy at the full budget.}\label{fig:pareto}
  \end{subfigure}
  \caption{Data efficiency and the cost--accuracy trade-off. The \Mfno/\Mgnn{}
           curves cross around $n\approx70$--$100$ training simulations;
           \Mtrans{} leads everywhere.}
...
Figure~\ref{fig:active_pool} shows the result before any solver runs: the
...
\begin{widefigure}
  \begin{subfigure}{0.48\linewidth}\centering
    ...
    \caption{Pool and the three arms.}\label{fig:active_pool}
  \end{subfigure}\hfill
  \begin{subfigure}{0.48\linewidth}\centering
    \includegraphics[width=\linewidth]{figures/fig11_active_verify.pdf}
    \caption{Surrogate error by arm on the accepted set.}\label{fig:active_verify}
  \end{subfigure}
...
$\aoa=18\degrees$ picks genuinely settled} (converged $C_D=0.167$ with
drift $0.2\%$, and quasi-steady $C_D=0.113$); and the remaining 17 ran their
full 7000 iterations at bounded, physically plausible post-stall magnitudes
(final $C_D\approx0.06$--$0.17$) while still drifting $7$--$21\%$ over the
last 500 iterations, so they are classified non-settling: past the envelope
...
\textbf{Post-stall (n=2, the settled $\aoa=18\degrees$ picks):} the
surrogates predict attached-flow drag ($\Chead\approx0.03$--$0.06$) where
the settled Fluent values are $0.113$ and $0.167$: errors of
...
cases the surrogate fails on} is now supported with hard numbers: the two
$\aoa=18\degrees$ picks the solver settled carry surrogate errors two to
three orders above any moderate case, and the other 17 picks sit where the
steady solver itself does not settle, the strongest possible form of ``the
surrogate should not be trusted here.'' The half that says \emph{random picks
...
Figure~\ref{fig:active_verify}.

\begin{widetable}
  \input{tables/tab5_summary}
\end{widetable}
...
on the two hardest single-axis shifts (\S\ref{sec:results_consistency}). The
...
each other and only modestly ahead of a 29-parameter ridge baseline, and on
...
leader is not the consistency leader on the shifts where it matters; and
...
\begin{widetable}
  \input{tables/tab5_cases}
\end{widetable}

\begin{widetable}
  \input{tables/tab5_selected_cases}
\end{widetable}
...
of the 110 committed \texttt{results/<run\_id>/metrics.json} files that carry
