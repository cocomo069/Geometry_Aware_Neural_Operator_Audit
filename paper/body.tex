...
\begin{widefigure}
  ...
  \caption{Active learning: (a) the 630-candidate pool against $\aoa$,
           ...
           accepted set, by arm and regime; each tick label carries that
           arm's accepted/non-settling counts, and the shaded band is
...
$\aoa=5\degrees$, three levels (Table~\ref{tab:gridstudy}).

% A float, not an inline center tabular: REVTeX's two-column output routine
% (ltxgrid) displaced the bottom rules of an inline booktabs tabular landing
% at a column bottom, striking through the last row. Floats are atomic; a
% widetable so all appendix tables pack together on shared float pages.
\begin{widetable}
  \caption{Grid-independence study: NACA0012, $\Rey=3\times10^6$,
    $\aoa=5\degrees$, SA closure.}
  \label{tab:gridstudy}
  \tblfont
  \begin{tabular}{lrrrr}
    \toprule
...
% Flush THIS appendix's four table* floats here, not at the bibliography:
% a barrier right before \prebib stranded the References heading at a fresh
% page top, where ltxgrid typesets it full-width and doubles the separator
% rule. With the flush here, Appendix C and the References heading flow
% in-column, exactly like the dg_trixi_shock reference paper.
\FloatBarrier
...
a lower bound on total compute, not an estimate (Table~\ref{tab:compute}).
...
leg (6 cores). Energy accounting is not available from the Kaggle API.

% A plain column float with [!htb], placed after Appendix C's single merged
% paragraph: after the Appendix B flush there are no pending floats, so it
% sits in-column and the References heading flows on after it. Kept as a
% float, never an inline tabular (ltxgrid slices inline booktabs rules at a
% column bottom), and the paragraph is one block so the underfull column's
% slack falls to the bottom instead of stretching the section skip.
\begin{table}[!htb]
  \centering
  \caption{Training compute, summed over every committed run, by model.}
  \label{tab:compute}
  \tblfont
  \begin{tabular}{lrr}
    \toprule
