...
% Also a widetable: a single-column float cannot share a float page with the
% table* floats, so as a plain table it stranded alone on a near-blank page.
\begin{widetable}
...
% Flush every deferred appendix float BEFORE the references start, so no
% appendix table lands on (or past) the bibliography pages. \FloatBarrier
% rather than \clearpage: it lets the references flow on after the floats
% instead of stranding a mostly-blank page in the onecolumn build.
\FloatBarrier
